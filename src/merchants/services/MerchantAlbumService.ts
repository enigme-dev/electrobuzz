...
export async function addMerchantAlbums(merchantId: string, data: AlbumsSchema) {
  let images = [];
  ...
  try {
    ...
    const albums = images.map((image) => ({merchantId: merchantId, albumPhotoUrl: image}));
...
export async function deleteMerchantAlbum(merchantId: string, merchantAlbumId: string) {
