import {IdentityStatuses, RegisterMerchantSchema} from "@/merchants/types";
...
export async function registerMerchant(userId: string, data: RegisterMerchantSchema) {
  let images = [];
  ...
  try {
    ...
    data.merchantIdentity.identityKTP = await uploadImg(
      encryptedKtp,
      {
        filename: `ktp-${userId}`,
        bucket: "vault",
    ...
    const encryptedSkck = await encrypt(
      data.merchantIdentity.identitySKCK
    );
    data.merchantIdentity.identitySKCK = await uploadImg(
      encryptedSkck,
      {
